...
func (p *SybasePlugin) Ports() []uint16 {
	return []uint16{5000}
}

// Run executes the Sybase ASE fingerprinting logic
func (p *SybasePlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
...
func DetectSybase(conn net.Conn, timeout time.Duration) (Data, bool, error) {
	...
	if len(response) == 0 {
		return Data{}, false, &shared.ServerNotEnable{}
	}
