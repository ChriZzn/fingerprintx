...
// TestPorts verifies that port 3868 is included in the default Diameter ports
func TestPorts(t *testing.T) {
	...
	if !found {
		t.Errorf("Ports() = %v, want to contain 3868", ports)
	}
}
...
func TestRunWithValidCEA(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			service, err := plugin.Run(&plugins.FingerprintConn{Conn: conn}, 5*time.Second, target)
			if err != nil {
			...
			if err := json.Unmarshal(service.Metadata, &metadata); err != nil {
				t.Fatalf("Failed to unmarshal metadata: %v", err)
			}

			if metadata.Version != tt.expectedVersion {
				t.Errorf("metadata.Version = %s, want %s", metadata.Version, tt.expectedVersion)
			}
