...
func (p *DIAMETERPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
	...
	if err != nil {
		...
		return plugins.CreateServiceFrom(target, p.Name(), metadata, conn.TLS()), nil
	}
...
// Ports returns the default Diameter port (3868).
func (p *DIAMETERPlugin) Ports() []uint16 {
	return []uint16{3868}
}
...
func detectDiameter(conn net.Conn, timeout time.Duration) ([]byte, error) {
	...
	// Send CER
	response, err := shared.SendRecv(conn, cer, timeout)
	if err != nil {
		return nil, err
	}
...
func validateCEA(response []byte) error {
	...
	if len(response) < 60 {
		return &shared.InvalidResponseErrorInfo{
			Service: DIAMETER,
