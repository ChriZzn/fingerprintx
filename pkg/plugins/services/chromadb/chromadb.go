...
func detectChromaDB(conn net.Conn, target plugins.Target, timeout time.Duration) (string, bool, error) {
	// Build host string for HTTP Host header
	host := fmt.Sprintf("%s:%d", target.Host, target.Address.Port())
...
func (p *ChromaDBPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
	version, detected, err := detectChromaDB(conn, target, timeout)
	if err != nil {
		return nil, err
	}
	if !detected {
...
func (p *ChromaDBPlugin) Ports() []uint16 {
	return []uint16{8000}
}
