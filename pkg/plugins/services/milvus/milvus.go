...
// ServiceMilvus holds metadata for Milvus detection
...
func tryGetVersionViaGRPC(target string, timeout time.Duration) (string, bool, error) {
	...
	conn, err := shared.GRPCDialWithTimeout(target, timeout)
	if err != nil {
		return "", false, err
	}
	defer conn.Close()
	...
	response, err := shared.GRPCInvokeUnary(conn, method, request, timeout)
	if err != nil {
...
func (p *MilvusPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
...
// Ports returns the default ports for Milvus database service
func (p *MilvusPlugin) Ports() []uint16 {
	return []uint16{19530}
}
