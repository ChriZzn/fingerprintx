...
// ServiceMilvusMetrics holds metadata for Milvus metrics detection
...
func (p *MilvusMetricsPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
...
// Ports returns the default ports for Milvus Prometheus metrics endpoint
func (p *MilvusMetricsPlugin) Ports() []uint16 {
	return []uint16{9091}
}
