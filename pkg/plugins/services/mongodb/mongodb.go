...
func checkMongoDBMsgResponse(response []byte, expectedRequestID uint32) (bool, error) {
	...
	if len(response) < 26 {
		return false, &shared.InvalidResponseErrorInfo{
			Service: MONGODB,
...
func tryGetMongoDBVersion(conn net.Conn, timeout time.Duration) string {
	...
	response, err := shared.SendRecv(conn, buildInfoQuery, timeout)
	...
	response, err = shared.SendRecv(conn, buildInfoMsg, timeout)
	if err == nil && len(response) > 0 {
...
func tryMongoDBMsgProtocol(conn net.Conn, timeout time.Duration) (mongoDBMetadata, bool, error) {
	...
	response, err := shared.SendRecv(conn, helloMsg, timeout)
	if err != nil {
		return mongoDBMetadata{}, false, err
	}
	if len(response) == 0 {
		return mongoDBMetadata{}, true, &shared.ServerNotEnable{}
	}
	...
	response, err = shared.SendRecv(conn, isMasterMsg, timeout)
	if err != nil {
		return mongoDBMetadata{}, false, err
	}
	if len(response) == 0 {
	...
	isMongoDB, err = checkMongoDBMsgResponse(response, requestID)
	if !isMongoDB {
		return mongoDBMetadata{}, true, &shared.InvalidResponseError{Service: MONGODB}
	}
...
func tryMongoDBQueryProtocol(conn net.Conn, timeout time.Duration) (mongoDBMetadata, bool, error) {
	...
	response, err := shared.SendRecv(conn, helloQuery, timeout)
	if err != nil {
		return mongoDBMetadata{}, false, err
	}
	if len(response) == 0 {
	...
	response, err = shared.SendRecv(conn, isMasterQuery, timeout)
	if err != nil {
		return mongoDBMetadata{}, false, err
	}
	if len(response) == 0 {
...
func DetectMongoDB(conn net.Conn, timeout time.Duration) (mongoDBMetadata, bool, error) {
	...
	return mongoDBMetadata{}, false, &shared.InvalidResponseError{Service: MONGODB}
}
...
func (p *MONGODBPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
	metadata, check, err := DetectMongoDB(conn, timeout)
	if check && err != nil {
		return nil, nil
	} else if check && err == nil {
		payload := ServiceMongoDB{
