...
//
// Returns:
//   - string: Version string (empty if not detected or version unavailable)
//   - bool: true if CouchDB detected, false otherwise
//   - error: Error details if detection failed
func detectCouchDB(conn net.Conn, target plugins.Target, timeout time.Duration) (string, bool, error) {
	...
	response, err := shared.SendRecv(conn, []byte(request), timeout)
	if err != nil {
		return "", false, err
	}
...
func (p *COUCHDBPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
	version, detected, err := detectCouchDB(conn, target, timeout)
	if err != nil {
		return nil, err
	}
	if !detected {
...
func (p *COUCHDBPlugin) Ports() []uint16 {
	return []uint16{5984, 6984}
}
