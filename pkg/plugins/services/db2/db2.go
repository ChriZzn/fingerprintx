...
func DetectDB2(conn net.Conn, timeout time.Duration) (db2Metadata, bool, error) {
	...
	response, err := shared.SendRecv(conn, excsat, timeout)
	if err != nil {
		return db2Metadata{}, false, err
	}

	if len(response) == 0 {
		return db2Metadata{}, false, &shared.InvalidResponseError{Service: DB2}
	}
...
func (p *DB2Plugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
