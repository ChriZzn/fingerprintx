...
// ServiceFirebird holds metadata for Firebird SQL server detection
...
func (p *FirebirdPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
	// Phase 1: Detection - Send op_connect packet
	connectPacket := buildConnectPacket()

	response, err := shared.SendRecv(conn, connectPacket, timeout)
	if err != nil {
		return nil, err
	}
...
// Ports returns the default ports for Firebird SQL server
func (p *FirebirdPlugin) Ports() []uint16 {
	return []uint16{3050}
}
