...
// ServiceRMI holds metadata for Java RMI detection
...
func (p *RMIPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
...
func detectRMI(conn *plugins.FingerprintConn, timeout time.Duration) (bool, string, error) {
	...
	response, err := shared.SendRecv(conn, handshake, timeout)
	if err != nil {
		return false, "", err
	}
...
// Ports returns the default ports for Java RMI services
func (p *RMIPlugin) Ports() []uint16 {
	return []uint16{1099}
}
