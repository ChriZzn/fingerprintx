...
func DetectCassandra(conn net.Conn, timeout time.Duration) (cassandraMetadata, bool, error) {
	...
	response, err := shared.SendRecv(conn, optionsFrame, timeout)
	if err != nil {
		return cassandraMetadata{}, false, err
	}

	if len(response) == 0 {
		return cassandraMetadata{}, false, &shared.ServerNotEnable{}
	}
...
func (p *CassandraPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
...
// Ports returns the default Cassandra CQL port (9042).
func (p *CassandraPlugin) Ports() []uint16 {
	return []uint16{9042}
}
