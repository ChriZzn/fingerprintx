...
func DetectNeo4j(conn net.Conn, timeout time.Duration) (string, bool, error) {
	...
	if err := shared.Send(conn, handshake, timeout); err != nil {
		return "", false, err
	}
	...
	if err := shared.Send(conn, buildHelloMessage(), timeout); err != nil {
		return "", false, err
	}
	...
	if selectedMajor >= 5 && err == nil && serverStr == "" {
		if err := shared.Send(conn, buildLogonMessage(), timeout); err != nil {
			return "", false, err
		}
...
func (p *NEO4JPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
...
func checkBoltHandshakeResponse(response []byte, proposals []uint32) (uint32, bool, error) {
	if len(response) < 4 {
		return 0, false, &shared.InvalidResponseErrorInfo{
			Service: NEO4J,
	...
	if _, ok := offered[version]; !ok {
		return version, false, &shared.InvalidResponseErrorInfo{
			Service: NEO4J,
...
func recvExact(conn net.Conn, n int, timeout time.Duration) ([]byte, error) {
	...
	for read < n {
		if err := conn.SetReadDeadline(time.Now().Add(timeout)); err != nil {
			return []byte{}, &shared.ReadTimeoutError{WrappedError: err}
		}
...
func recvBoltMessageRaw(conn net.Conn, timeout time.Duration) ([]byte, error) {
	...
	for {
		...
		if len(raw) >= maxMessageBytes {
			return []byte{}, &shared.InvalidResponseErrorInfo{
				Service: NEO4J,
