...
func (p *SMPPPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
...
func detectSMPP(conn net.Conn, timeout time.Duration) (bool, []byte, error) {
	...
	response, err := shared.SendRecv(conn, enquireLinkPDU, timeout)
	...
	bindResponse, bindErr := shared.SendRecv(conn, bindPDU, timeout)
	if bindErr != nil {
	...
	return false, nil, &shared.InvalidResponseError{Service: SMPP}
}
...
func enrichSMPP(conn net.Conn, timeout time.Duration, cachedBindResp []byte) (string, string, VendorInfo) {
	...
	if len(cachedBindResp) >= MIN_RESPONSE_SIZE {
		...
		bindPDU := buildBindTransceiverPDU()
		var err error
		response, err = shared.SendRecv(conn, bindPDU, timeout)
