...
// ServiceMemcached holds metadata for Memcached detection
...
func DetectMemcached(conn net.Conn, timeout time.Duration) (string, bool, error) {
	...
	response, err := shared.SendRecv(conn, versionCmd, timeout)
	if err != nil {
		return "", false, err
	}
	if len(response) == 0 {
		return "", false, &shared.ServerNotEnable{}
	}
	...
	response, err = shared.SendRecv(conn, statsCmd, timeout)
	if err != nil {
		return "", false, err
	}
	if len(response) == 0 {
	...
	return "", false, &shared.InvalidResponseError{Service: MEMCACHED}
}

func (p *MEMCACHEDPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
...
// Ports returns the default ports for Memcached
func (p *MEMCACHEDPlugin) Ports() []uint16 {
	return []uint16{11211}
}
