...
func (p *PINECONEPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
	...
	if err != nil {
		...
		return nil, &shared.RequestError{Message: err.Error()}
	}
	...
	if apiVersion != "" {
		return p.buildDetectionResult(target, conn, resp, apiVersion, "high")
	}

	// SECONDARY detection: x-pinecone-auth-rejected-reason (fallback)
	if authRejected != "" {
		return p.buildDetectionResult(target, conn, resp, "", "medium")
	}
...
// Type returns TCP since the fork doesn't have TCPTLS.
...
func (p *PINECONEPlugin) Ports() []uint16 {
	return []uint16{443}
}
