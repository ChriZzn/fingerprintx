...
func detectElasticsearch(conn net.Conn, timeout time.Duration) (string, bool, error) {
	...
	response, err := shared.SendRecv(conn, []byte(httpRequest), timeout)
	if err != nil {
		return "", false, err
	}
	if len(response) == 0 {
		return "", false, &shared.InvalidResponseError{Service: ELASTICSEARCH}
	}
...
func (p *ElasticsearchPlugin) Run(conn *plugins.FingerprintConn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
...
func (p *ElasticsearchPlugin) Ports() []uint16 {
	return []uint16{9200}
}
