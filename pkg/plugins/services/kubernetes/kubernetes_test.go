...
// TestKubernetesPlugin_Ports tests that the plugin returns correct default ports
func TestKubernetesPlugin_Ports(t *testing.T) {
	plugin := &KubernetesPlugin{}

	ports := plugin.Ports()
	if len(ports) != 2 {
		t.Fatalf("Ports() returned %d ports, expected 2", len(ports))
	}
	if ports[0] != 6443 || ports[1] != 8443 {
		t.Errorf("Ports() = %v, expected [6443 8443]", ports)
	}
}
