...
// ServiceKubernetes holds metadata for Kubernetes API server detection
...
// Ports returns the default ports for Kubernetes API server
func (p *KubernetesPlugin) Ports() []uint16 {
	return []uint16{6443, 8443}
}
...
func checkKubernetesVersion(data []byte) (VersionInfo, error) {
	...
	if err != nil {
		return VersionInfo{}, &shared.InvalidResponseErrorInfo{
			Service: KUBERNETES,
